---
title: 조코딩 웹개발 5주 완성 1주차 리뷰
date: 2022-07-25 15:00:00 +0900
categories: [조코딩, 웹개발 5주 완성]
tags: [조코딩, 웹개발 5주 완성, 1주차, 리뷰]     # TAG names should always be lowercase
math: true
toc: false
---
...
다음 링크의 <a href="https://youtu.be/Ct-lOOUqmyY">**영상**</a>은 **샌드위치 코딩**이라는 영상으로 샌드위치를 만드는 과정을 구체적이고 논리적으로 아이들한테 설명해 보도록 만드는 영상입니다.
...
이처럼 `h1~6`, `p` 태그들을 이용하여 본문의 제목, 내용을 지정할 수 있습니다.
...
`img` 태그에서 `alt` 속성은 이미지를 불러오지 못했을때 대신 띄우는 문구를 의미합니다.
`src` 속성은 이미지의 경로를 의미합니다.
본문의 거위 사진은 제가 오늘 낮에 찍은 사진으로 깃헙에 업로드 후 [해당 이미지의 웹 페이지 url](https://user-images.githubusercontent.com/64826387/180761565-8aa04f19-711d-43b6-843c-e1abe4b4a919.jpg)을 `src` 속성으로 지정해줬습니다.
...
다음은 `input` 태그입니다.
`input` 태그의 `placeholder` 속성은 해당 입력창에 들어가야될 내용의 힌트를 지정합니다.
`type` 속성은 해당 입력창의 타입을 지정합니다.
`value` 속성은 해당 입력창의 값을 지정합니다.
...
다음은 `a` 태그입니다.
`a` 태그의 `href` 속성은 웹 페이지의 url을 입력하여 태그 안의 내용을 입력한 웹 페이지의 하이퍼 링크로 지정해줄 수 있습니다.
...
이 외에도 본 포스팅의 초반에 **조코딩**님의 유튜브 영상을 퍼올때 쓴 `iframe` 태그 등 다양한 태그들이 **HTML**에 존재하는데 더 다양한 태그들이 궁금하신 분들은 다음의 블로그를 참조해주시기 바랍니다: <a href="https://pridiot.tistory.com/6">https://pridiot.tistory.com/6</a>

## Deploy (웹 사이트 배포)

그러면 여태까지 배운 내용들을 활용하여 웹 페이지를 만들어 실제 url을 가진 웹 사이트로 배포해보겠습니다.
배포에는 [Netlify](https://www.netlify.com/)라는 웹 서비스를 사용하였고, 템플릿은 [Free CSS](https://www.free-css.com/)라는 웹 사이트를 사용하였습니다.

<a href="https://www.free-css.com/free-css-templates/page281/john-doe"><img width="1717" alt="템플릿" src="https://user-images.githubusercontent.com/64826387/180778915-841775ce-06fd-4fcc-93cc-d636541c690f.PNG"></a>
...
2. 상단의 메뉴 중 `Sites`에 들어갑니다.
<img width="933" alt="Netlify 사이트 배포 1" src="https://user-images.githubusercontent.com/64826387/180784714-e2d9a1cc-cd9e-4537-9f1b-a73f8b032062.PNG">
3. 위 이미지 속 드랍박스에 `index.html`이 포함된 폴더 (저와 같은 템플릿으로 진행하셨을 경우 `public_html` 폴더)를 드래그 앤 드랍해줍니다.
4. 웹 사이트 배포가 완료되었다면 `Site settings`-`Change site name`을 통해 도메인을 커스터마이징할 수 있습니다.
5. 웹 사이트 수정을 원하시는 경우, 배포한 홈페이지 메뉴의 `Deploys`에 들어가 드랍박스에 내용이 수정된 폴더를 드래그 앤 드랍해줍니다.

실제 배포한 웹 사이트: [https://duckbankbok.netlify.app/](https://duckbankbok.netlify.app/)

제가 배포한 웹 사이트의 경우 실습에 사용된 템플릿이 커스터마이징할 수 있는 부분이 너무 많아 웹 사이트의 header 부분만 커스터마이징 해봤습니다. 긴 글 읽어주셔서 정말 감사합니다:)